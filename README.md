...
| 行 | 内容 | 说明 |
|---|---|---|
| 第一行 | Codex 状态灯 + 状态文字 | 红灯表示空闲，绿灯表示正在工作 / 自动审查 / 等待用户，蓝灯表示任务已完成。 |
| 第二行 | 5h 额度进度条 | 显示当前 5 小时窗口剩余额度。 |
| 第三行 | 7d 额度进度条 | 显示当前 7 天窗口剩余额度。 |

状态条支持左键打开详情、右键打开菜单；未锁定位置时可以拖动，并可自动吸附到任务栏通知区域附近。
...
- **Codex 活动识别**：从本地 Codex session 事件识别空闲、工作中、自动审查、等待用户、已完成、错误等状态。
- **双额度窗口**：同时显示 5 小时额度和 7 天额度的剩余比例。
